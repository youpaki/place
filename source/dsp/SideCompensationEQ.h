...
class SideCompensationEQ
{
public:
    SideCompensationEQ() = default;
    ...
    void process (float* sideChannel, int numSamples, float amount) noexcept
    {
...
private:
    ...
    static constexpr std::array<float, numBands> bandFreqs = { 500.0f, 2500.0f, 5000.0f, 8000.0f };
    static constexpr std::array<float, numBands> maxBoostDB = { 2.0f, 5.0f, 3.0f, 2.0f };
    static constexpr std::array<float, numBands> qValues = { 1.2f, 1.5f, 1.2f, 1.0f };
    static constexpr float compensationScale = 0.4f;
    ...
    void updateCoefficients (double sampleRate, float amount = -1.0f)
    {
        ...
        {
            float gainDB = maxBoostDB[i] * currentAmount * compensationScale;
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (SideCompensationEQ)
};
